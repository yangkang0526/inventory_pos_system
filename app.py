...
def add_product():
    ...
    product_id = db.add_product(
        ...
        float(data['price']), float(data['cost']),
        int(data['quantity']), int(data['min_quantity']), data['unit']
    )
...
def stock_in():
    data = request.json
    db.add_stock_in(
        data['product_id'], int(data['quantity']),
        float(data.get('cost_price', 0)), data.get('supplier', ''), data.get('remark', '')
    )
...
def add_sale():
    ...
    if product[6] < data['quantity']:
        return jsonify({'success': False, 'error': f'{product[2]} 库存不足'})
    
    db.add_sale(
        order_no, data['product_id'], data['quantity'],
        float(data['price']), float(data['total']),
        data.get('payment_method', '现金'), data.get('customer_name', '散客'), data.get('remark', '')
    )
...
def get_sales_report():
    ...
    return jsonify([{
        'id': s[0], 'order_no': s[1], 'quantity': s[3], 'price': s[4],
        'total': s[5], 'payment_method': s[6], 'customer_name': s[7],
        'created_at': s[9], 'product_name': s[11], 'product_code': s[10]
    } for s in sales])
...
def get_stock_in_report():
    ...
    return jsonify([{
        'id': s[0], 'quantity': s[2], 'cost_price': s[3],
        'supplier': s[4], 'created_at': s[6], 'product_name': s[8], 'product_code': s[7]
    } for s in stock_in])
...
def get_low_stock():
    ...
    return jsonify([{
        'id': p[0], 'code': p[1], 'name': p[2], 'quantity': p[6], 'min_quantity': p[7]
    } for p in products])
